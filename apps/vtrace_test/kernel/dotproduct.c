...
int64_t dotp_v64b_mask(int64_t *a, int64_t *b, uint64_t avl) {
#ifdef INTRINSICS
  ...
  // Mask: elements 0-3 active, elements 4+ masked
  vuint64m8_t idx = vid_v_u64m8(vl);
  vbool8_t mask = vmsltu_vx_u64m8_b8(idx, 4, vl);
  ...
  for (; avl > 0; avl -= vl) {
    ...
    // Multiply and accumulate
    if (avl == orig_avl) {
      acc = vmul_vv_i64m8_m(mask, acc, buf_a, buf_b, vl);
    } else {
      acc = vmacc_vv_i64m8_m(mask, acc, acc, buf_a, buf_b, vl);
    }
  ...
  // Mask: elements 0-3 active, elements 4-7 masked.
  // v0 is the mask register.
  //
  // We generate:
  //   v0[3:0] = 1
  //   v0[7:4] = 0
  //
  // by comparing element indices against 4.
  asm volatile(
      "vid.v    v0\n"
      "vmsltu.vx v0, v0, %0"
      :
  ...
  for (; avl > 0; avl -= vl) {
    ...
    // Re-create mask after vsetvl in case vl changed.
    asm volatile(
        "vid.v     v0\n"
        "vmsltu.vx v0, v0, %0"
        :
    ...
    // Multiply and accumulate, using v0 as mask.
    if (avl == orig_avl) {
      asm volatile("vmul.vv v24, v8, v16, v0.t");
    } else {
      asm volatile("vmacc.vv v24, v8, v16, v0.t");
    }
